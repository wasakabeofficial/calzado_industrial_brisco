...
export default function CallEndReasonChart({ leads }: CallEndReasonChartProps) {
  const chartData = useMemo(() => {
    ...
    leads.forEach((lead) => {
      ...
      const label = razon.toLowerCase().trim().replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase());
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-4 md:p-6">
      ...
      <div className="flex items-end justify-center gap-2 md:gap-3" style={{ height: `${BAR_MAX_HEIGHT}px` }}>
        {chartData.map((item) => {
          const barHeight = item.count > 0 ? Math.max((item.count / maxCount) * (BAR_MAX_HEIGHT - 20), 8) : 0;
          return (
            <div key={item.label} className="flex flex-col items-center gap-1 group relative shrink-0">
              ...
              <span className="text-[9px] md:text-[10px] text-gray-400 text-center max-w-16 md:max-w-20 leading-tight truncate">{item.label}</span>
            </div>
          );
        })}
      </div>
