...
describe('useLeadTranscription Hook', () => {
  const mockTranscriptionText = 'Esta es una transcripción de prueba de la llamada.'
  ...
  it('fetches transcription successfully', async () => {
    vi.mocked(leadService.getLeadTranscription).mockResolvedValue(mockTranscriptionText)
  ...
  it('handles error when fetch fails', async () => {
    vi.mocked(leadService.getLeadTranscription).mockRejectedValue(new Error('Fetch failed'))
